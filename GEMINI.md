...
read-if: "you are Gemini starting work in this repo"
skip-if: "never"
---

# Gemini — Project Adapter
...
- Work log: `docs/agents/gemini.md`
...
When Gemini finishes a handoff-worthy chunk (e.g., branch complete, major refactor done, cross-cutting change that needs review), write a handoff block:

```
./scripts/collab-handoff.sh <to-agent> --from gemini --message "..." --files "a b c"
```

When the user says "take the baton" or "pick up handoff," run:

```
./scripts/collab-catchup.sh preview --agent gemini --handoff
```

…and follow the instructions in the surfaced handoff block. After validation, close the handoff:

```
./scripts/collab-handoff.sh close <id> --from gemini
```