#!/usr/bin/env node
// V7 -> V8 deterministic row-level diff for shadow production-filter audit
// workbooks. Mirrors scripts/v6-v7-diff.mjs and encodes V8-specific
// likely-cause inferences (V8-A1 strict-NA territory + reason rename;
// V8-A2 director sales titles + Regional Sales Manager; V8-A3 CSM rule
// with carve-out; V8-A4 Workday language-switcher hygiene).
//
// Acts as the regression-baseline gate: every V7→V8 hard_drop status flip
// AND reason-string change is tagged to V8-A1/A2/A3/A4 / OTHER. Reason
// renames (non_na_territory_with_sales_context → non_na_territory) are
// tagged "V8-A1: reason rename" rather than "OTHER".
...
const DEFAULT_OUTPUT_XLSX = resolve(CAREER_OPS, "output", "production-filter-refinement-v7-v8-diff.xlsx");
const DEFAULT_SUMMARY_JSON = resolve(REPO_ROOT, "docs", "audits", "2026-05-06-shadow-v7-v8-diff-summary.json");
...
const SHEET_NAMES = [
  "Diff Manifest",
  "V7 to V8 Changed Rows",
  "Hard Drop Added",
  "Hard Drop Removed",
  "Hard Drop Reason Changed",
  "V8-A1 Strict NA Territory",
  "V8-A2 Director Sales",
  "V8-A3 CSM Carve Out",
  "V8-A4 Source Hygiene",
...
export function parseArgs(argv) {
  ...
  for (let i = 2; i < argv.length; i++) {
    ...
    if (arg === "--run-date") flags.runDate = argv[++i];
      ...
      console.log("Usage: node scripts/v7-v8-diff.mjs [--allow-overwrite] [--v7 PATH] [--v8 PATH]");
      process.exit(0);
    } else {
...
// V8 likely-cause classifier. Inspects reason-token delta and field changes to
// tag the responsible V8-Ax change. Multiple causes possible; joined by "; ".
export function classifyV8Cause(fromRecord, toRecord, changes) {
  ...
  const newSourceRepair = toRecord?.source_repair_reason || "";

  // V8-A1: reason rename — non_na_territory_with_sales_context → non_na_territory.
  if (delta.removed.includes("non_na_territory_with_sales_context") && delta.added.includes("non_na_territory")) {
    causes.add("V8-A1_reason_rename");
  }
  // V8-A1: new strict-NA drop (territory drop added without it being a rename).
  if (delta.added.includes("non_na_territory") && !delta.removed.includes("non_na_territory_with_sales_context")) {
    causes.add("V8-A1_strict_na_territory");
  }
  // V8-A2: director-level sales titles or Regional Sales Manager dropping on sales_role_title.
  if (delta.added.includes("sales_role_title") &&
      /\b(account director|sales director|director,?\s+sales|director\s+of\s+sales|regional sales)\b/i.test(oldTitle)) {
    causes.add("V8-A2_director_sales");
  }
  // V8-A3: CSM titles dropping on sales_role_title.
  if (delta.added.includes("sales_role_title") &&
      /\b(customer success\s+(?:manager|director|lead|head)|renewals?\s+(?:manager|specialist|director))\b/i.test(oldTitle)) {
    causes.add("V8-A3_csm");
  }
  // V8-A4: Workday language-switcher chrome routing.
  if (/workday_language_switcher_chrome/.test(newSourceRepair)) {
    causes.add("V8-A4_workday_chrome");
  }
...
function classifyChange(fromRecord, toRecord, changes) {
  ...
  const v8Causes = classifyV8Cause(fromRecord, toRecord, changes);
  return {
    change_type: changeTypes.join("; "),
    v8_cause: v8Causes.join("; "),
  };
}
...
function diffVersions(fromVersion, toVersion) {
  ...
  for (const key of [...allKeys].sort()) {
    ...
    const { change_type, v8_cause } = classifyChange(fromRecord, toRecord, changes);
...
function buildCompanyPivot(rows) {
  return [...groupBy(rows, (r) => r.company).entries()]
    .map(([company, group]) => ({
      ...
      v8a1_count: countWhere(group, (r) => /V8-A1/.test(r.v8_cause)),
      v8a2_count: countWhere(group, (r) => /V8-A2/.test(r.v8_cause)),
      v8a3_count: countWhere(group, (r) => /V8-A3/.test(r.v8_cause)),
      v8a4_count: countWhere(group, (r) => /V8-A4/.test(r.v8_cause)),
      other_count: countWhere(group, (r) => /OTHER/.test(r.v8_cause)),
      titles: group.map((r) => r.title).join(" | "),
    }))
    .sort((a, b) => b.changed_rows - a.changed_rows || a.company.localeCompare(b.company));
}

function buildSummaryMetrics(v7, v8, diff, unmatchedRows) {
  const changed = diff.changedRows;
  return [
    { metric: "from_shadow_rows", value: v7.counts.shadow_rows },
    { metric: "to_shadow_rows", value: v8.counts.shadow_rows },
    { metric: "shadow_rows_dedup_delta", value: v8.counts.shadow_rows - v7.counts.shadow_rows },
    ...
    { metric: "v8_a1_attributed_rows", value: countWhere(changed, (r) => /V8-A1/.test(r.v8_cause)) },
    { metric: "v8_a2_attributed_rows", value: countWhere(changed, (r) => /V8-A2/.test(r.v8_cause)) },
    { metric: "v8_a3_attributed_rows", value: countWhere(changed, (r) => /V8-A3/.test(r.v8_cause)) },
    { metric: "v8_a4_attributed_rows", value: countWhere(changed, (r) => /V8-A4/.test(r.v8_cause)) },
    { metric: "v8_other_unattributed_rows", value: countWhere(changed, (r) => /OTHER/.test(r.v8_cause)) },
    ...
    { metric: "from_hard_drops", value: v7.counts.hard_drops },
    { metric: "to_hard_drops", value: v8.counts.hard_drops },
    { metric: "net_hard_drop_delta", value: v8.counts.hard_drops - v7.counts.hard_drops },
    { metric: "from_sales_hard_drops", value: v7.counts.sales_hard_drops },
    { metric: "to_sales_hard_drops", value: v8.counts.sales_hard_drops },
    { metric: "from_territory_hard_drops", value: v7.counts.territory_hard_drops },
    { metric: "to_territory_hard_drops", value: v8.counts.territory_hard_drops },
    { metric: "v8_workday_chrome_routes", value: v8.counts.workday_chrome_routes },
    { metric: "from_validation_findings", value: v7.counts.validation_findings },
    { metric: "to_validation_findings", value: v8.counts.validation_findings },
...
export async function buildVersionDiff(flags) {
  ...
  const diff = diffVersions(versions.v7, versions.v8);
  const summaryMetrics = buildSummaryMetrics(versions.v7, versions.v8, diff, diff.unmatchedRows);
  const companyPivot = buildCompanyPivot(diff.changedRows);

  const wb = new ExcelJS.Workbook();
  wb.creator = "career-ops shadow v7-v8 diff";
  ...
  addSheet(wb, "V7 to V8 Changed Rows", diff.changedRows, changedCols);
  ...
  addSheet(wb, "V8-A1 Strict NA Territory", diff.changedRows.filter((r) => /V8-A1/.test(r.v8_cause)), changedCols);
  addSheet(wb, "V8-A2 Director Sales", diff.changedRows.filter((r) => /V8-A2/.test(r.v8_cause)), changedCols);
  addSheet(wb, "V8-A3 CSM Carve Out", diff.changedRows.filter((r) => /V8-A3/.test(r.v8_cause)), changedCols);
  addSheet(wb, "V8-A4 Source Hygiene", diff.changedRows.filter((r) => /V8-A4/.test(r.v8_cause)), changedCols);
  ...
  addSheet(wb, "Company Role Pivot", companyPivot, ["company", "changed_rows", "hard_drop_added", "hard_drop_removed", "hard_drop_reason_changed", "v8a1_count", "v8a2_count", "v8a3_count", "v8a4_count", "other_count", "titles"]);
