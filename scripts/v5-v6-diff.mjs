#!/usr/bin/env node
// V5 -> V6 deterministic row-level diff for shadow production-filter audit
// workbooks. Reuses ExcelJS plumbing patterns from shadow-version-diff.mjs but
// is a standalone sister script so it can encode V6-specific likely-cause
// inferences (sales policy 2, source-hygiene F-003/F-004, F-009 validation gate)
// without reshaping the V3/V4/V5 diff API.
// Reads generated V5/V6 artifacts only. No network, no production mutation.
...
const DEFAULT_OUTPUT_XLSX = resolve(CAREER_OPS, "output", "calibration", "diffs", "v5-v6.xlsx");
const DEFAULT_SUMMARY_JSON = resolve(REPO_ROOT, "docs", "audits", "2026-05-05-shadow-v5-v6-diff-summary.json");

const DEFAULT_VERSIONS = {
  v5: resolve(CAREER_OPS, "output", "calibration", "reviews-by-version", "v5.xlsx"),
  v6: resolve(CAREER_OPS, "output", "calibration", "reviews-by-version", "v6.xlsx"),
};
...
// V6 (F-002): row keys match by (normalized_url, company). V6 strips listing-
// page chrome from titles, so title-included keys would falsely split a
// stripped-title row from its V5 mirror. URL is unique per row in Shadow
// Decisions so URL+company is sufficient.
export function rowKey(row) {
  return [normalizeUrl(row.url), normalizeKeyPart(row.company)].join("\t");
}

// V5/V6 cross-version match: V6 dedups listing-mirror URLs, so for a V5 row on
// e.g. scale.com/careers/<id> the matching V6 row is on greenhouse.io/.../<id>.
// We expose a fallback gh_jid key for second-pass matching after URL fails.
function gh_jid(url) {
  const u = String(url || "");
  let m = /greenhouse\.io\/[^/]+\/jobs\/(\d{6,})/i.exec(u); if (m) return `gh:${m[1]}`;
  m = /scale\.com\/careers\/(\d{6,})/i.exec(u); if (m) return `gh:${m[1]}`;
  m = /accel\.com\/companies\/[^/]+\/jobs\/(\d{6,})/i.exec(u); if (m) return `gh:${m[1]}`;
...
function jidKey(row) {
  const j = gh_jid(row.url);
  if (!j) return "";
  return `${normalizeKeyPart(row.company)}\t${j}`;
}
...
export function parseArgs(argv) {
  ...
  for (let i = 2; i < argv.length; i++) {
    ...
    if (arg === "--run-date") flags.runDate = argv[++i];
      ...
      console.log("Usage: node scripts/v5-v6-diff.mjs [--allow-overwrite] [--v5 PATH] [--v6 PATH]");
      process.exit(0);
    } else {
...
async function readVersion(version, filePath) {
  ...
  for (const row of rowsBySheet["Shadow Decisions"]) {
    ...
    const jk = jidKey(record);
    if (jk && !recordsByJid.has(jk)) recordsByJid.set(jk, record);
  }

  return {
    version, filePath, sha256: sha256(filePath),
    sheetNames: workbook.worksheets.map((sheet) => sheet.name),
    records, recordsByJid, warnings,
    counts: {
...
function classifyChange(fromRecord, toRecord, changes) {
  ...
  // V6-specific: sales policy 2 movement (rows that changed sales_hard_drop status).
  if ((fromRecord?.sales_hard_drop || "no") !== (toRecord?.sales_hard_drop || "no")) {
    changeTypes.push("sales_policy_2_movement");
  }
  ...
  return { change_type: changeTypes.join("; "), likely_cause: inferLikelyCause(fromRecord, toRecord).join("; ") };
}

function inferLikelyCause(fromRecord, toRecord) {
  ...
  const oldHard = fromRecord?.hard_drop === "yes";
  const newHard = toRecord?.hard_drop === "yes";
  // V6 (F-001 / policy 2): sales hard drop removed for SA/FDE family.
  if (/sales_role_content/.test(oldReason) && !/sales_role_content/.test(newReason) && oldHard) {
    causes.add("v6_sales_policy_2_threshold_or_boilerplate_downweight_kept_row");
  }
  // V6 (F-008): sales_department alone no longer fires; row recovered.
  if (/sales_role_content/.test(oldReason) && !/sales_role_content/.test(newReason) && oldHard && !newHard) {
    causes.add("v6_sales_department_corroboration_required");
  }
  // V6 (F-003): generic_careers_index for /all-jobs?team= URLs.
  if (/generic_careers_index/.test(toRecord?.source_repair_reason || "") && !/generic_careers_index/.test(fromRecord?.source_repair_reason || "")) {
    causes.add("v6_atlassian_all_jobs_listing_routed_to_source_repair");
  }
  // V6 (F-004): placeholder_or_invalid_url.
  if (/placeholder_or_invalid_url/.test(toRecord?.source_repair_reason || "")) {
    causes.add("v6_placeholder_url_routed_to_source_repair");
  }
  // V6 (F-009): validation finding suppressed when location_reason empty.
  if ((fromRecord?.validation_finding || "") && !(toRecord?.validation_finding || "")) {
    causes.add("v6_specific_location_validation_finding_gated_off");
  }
  if ((fromRecord?.shadow_score || "") !== (toRecord?.shadow_score || "") || (fromRecord?.shadow_band || "") !== (toRecord?.shadow_band || "")) {
    causes.add("score_or_band_changed");
  }
  if ((fromRecord?.title || "") !== (toRecord?.title || "")) causes.add("v6_listing_title_chrome_stripped");
  if (!causes.size && reasonText.trim()) causes.add("material_field_changed");
  if (!causes.size) causes.add("supporting_field_changed");
  return [...causes];
}
...
function diffVersions(fromVersion, toVersion) {
  ...
  for (const key of [...allKeys].sort()) {
    let fromRecord = fromVersion.records.get(key);
    let toRecord = toVersion.records.get(key);
    // V6 (F-002): when V5 row has a listing-mirror URL that V6 dedup'd against
    // a canonical URL, the URL keys won't match. Fall back to (company, gh_jid)
    // matching to recover the cross-version pair.
    if (fromRecord && !toRecord) {
      const jk = jidKey(fromRecord);
      if (jk) {
        const cand = toVersion.recordsByJid.get(jk);
        if (cand && !matchedToKeys.has(cand.key)) toRecord = cand;
      }
    } else if (!fromRecord && toRecord) {
      const jk = jidKey(toRecord);
      if (jk) {
        const cand = fromVersion.recordsByJid.get(jk);
        if (cand) fromRecord = cand;
      }
    }
    // Skip V6-only iterations whose record was already matched on the V5 pass
    // via the JID fallback (avoids double-counting unmatched).
    if (!fromRecord && toRecord && matchedToKeys.has(toRecord.key)) continue;
    if (toRecord) matchedToKeys.add(toRecord.key);
    if (!fromRecord || !toRecord) {
      unmatchedRows.push({
        comparison: "v5_to_v6",
        ...
        reason: fromRecord ? `missing in ${toVersion.version} (likely deduped or removed)` : `missing in ${fromVersion.version}`,
      });
      continue;
    }
    const changes = fieldChanges(fromRecord, toRecord);
    if (!changes.length) continue;
    const { change_type, likely_cause } = classifyChange(fromRecord, toRecord, changes);
    ...
    changedRows.push({
      ...
      old_url: fromRecord.url,
      new_url: toRecord.url,
      ...
      old_validation_finding: fromRecord.validation_finding,
      new_validation_finding: toRecord.validation_finding,
      change_type, likely_cause,
...
function buildCompanyPivot(rows) {
  return [...groupBy(rows, (r) => r.company).entries()]
    .map(([company, group]) => ({
      ...
      sales_policy_2_movement: countWhere(group, (r) => /sales_policy_2_movement/.test(r.change_type)),
...
function buildSummaryMetrics(v5, v6, diff, unmatchedRows) {
  const changed = diff.changedRows;
  return [
    { metric: "from_shadow_rows", value: v5.counts.shadow_rows },
    { metric: "to_shadow_rows", value: v6.counts.shadow_rows },
    { metric: "shadow_rows_dedup_delta", value: v6.counts.shadow_rows - v5.counts.shadow_rows },
    ...
    { metric: "sales_policy_2_movement_rows", value: countWhere(changed, (r) => /sales_policy_2_movement/.test(r.change_type)) },
    ...
    { metric: "from_hard_drops", value: v5.counts.hard_drops },
    { metric: "to_hard_drops", value: v6.counts.hard_drops },
    { metric: "net_hard_drop_delta", value: v6.counts.hard_drops - v5.counts.hard_drops },
    { metric: "from_sales_hard_drops", value: v5.counts.sales_hard_drops },
    { metric: "to_sales_hard_drops", value: v6.counts.sales_hard_drops },
    { metric: "from_validation_findings", value: v5.counts.validation_findings },
    { metric: "to_validation_findings", value: v6.counts.validation_findings },
    { metric: "from_source_repair_review_shadow_rows", value: v5.counts.source_repair_review_shadow_rows },
    { metric: "to_source_repair_review_shadow_rows", value: v6.counts.source_repair_review_shadow_rows },
...
export async function buildVersionDiff(flags) {
  ...
  const diff = diffVersions(versions.v5, versions.v6);
  const summaryMetrics = buildSummaryMetrics(versions.v5, versions.v6, diff, diff.unmatchedRows);
  const companyPivot = buildCompanyPivot(diff.changedRows);

  const wb = new ExcelJS.Workbook();
  wb.creator = "career-ops shadow v5-v6 diff";
  ...
  const changedCols = [
    "comparison", "company", "title", "url", "old_url", "new_url", "old_hard_drop", "new_hard_drop",
    ...
    "old_reviewer_queue", "new_reviewer_queue", "old_source_repair", "new_source_repair",
    "old_source_repair_reason", "new_source_repair_reason", "old_sales_hard_drop", "new_sales_hard_drop",
    "old_validation_finding", "new_validation_finding", "change_type", "likely_cause",
    "changed_fields", "changed_field_count", "changed_values", "old_evidence", "new_evidence",
  ];

  addSheet(wb, "Diff Manifest", manifestRows, ["artifact", "path", "sha256", "rows", "hard_drops", "sales_hard_drops", "reviewer_queue", "source_repair_review", "source_repair_review_shadow_rows", "validation_findings", "warnings"]);
  addSheet(wb, "V5 to V6 Changed Rows", diff.changedRows, changedCols);
  ...
  addSheet(wb, "Sales Policy 2 Movement", diff.changedRows.filter((r) => /sales_policy_2_movement/.test(r.change_type)), changedCols);
  addSheet(wb, "Review Queue Movement", diff.changedRows.filter((r) => /review_queue_movement/.test(r.change_type)), changedCols);
  addSheet(wb, "Source Repair Movement", diff.changedRows.filter((r) => /source_repair_movement/.test(r.change_type)), changedCols);
  ...
  addSheet(wb, "Company Role Pivot", companyPivot, ["company", "changed_rows", "hard_drop_added", "hard_drop_removed", "hard_drop_reason_changed", "sales_policy_2_movement", "review_queue_movement", "source_repair_movement", "score_or_band_changed", "titles"]);
