#!/usr/bin/env node
// V8 -> V9 deterministic row-level diff for shadow production-filter audit
// workbooks. Mirrors scripts/v7-v8-diff.mjs and encodes V9-specific
// likely-cause inferences:
//   - V9-1: NA_CITIES_RE expansion (bare US-city abbrev guards SF/NY/LA/DC)
//           and NA_COAST_RE addition (US East Coast / US West Coast / coastal US)
//           — closes Round 5 confirmed FPs (Vercel x2, XBOW).
//   - V9-2: ROLE_ANCHOR_PATTERNS markdown ## Location block + role-base offices
//           pattern, plus location-typed section bodies as implicit anchors —
//           closes Round 5 named-cohort miss (Cohere FDE Infrastructure
//           Specialist).
//
// Acts as the regression-baseline gate: every V8→V9 hard_drop status flip
// AND reason-string change is tagged to V9-A1/A2 / OTHER.
...
const DEFAULT_OUTPUT_XLSX = resolve(CAREER_OPS, "output", "production-filter-refinement-v8-v9-diff.xlsx");
const DEFAULT_SUMMARY_JSON = resolve(REPO_ROOT, "docs", "audits", "2026-05-06-shadow-v8-v9-diff-summary.json");

const DEFAULT_VERSIONS = {
  v8: resolve(CAREER_OPS, "output", `production-filter-refinement-review-${DEFAULT_RUN_DATE}-v8.xlsx`),
...
const SHEET_NAMES = [
  "Diff Manifest",
  "V8 to V9 Changed Rows",
  "Hard Drop Added",
  "Hard Drop Removed",
  "Hard Drop Reason Changed",
  "V9-A1 NA Token Expansion",
  "V9-A2 Location Anchors",
...
export function parseArgs(argv) {
  ...
  for (let i = 2; i < argv.length; i++) {
    ...
    else if (arg === "--v8") flags.versions.v8 = resolve(REPO_ROOT, argv[++i]);
    else if (arg === "--v9") flags.versions.v9 = resolve(REPO_ROOT, argv[++i]);
    else if (arg === "--allow-overwrite") flags.allowOverwrite = true;
    else if (arg === "--help" || arg === "-h") {
      console.log("Usage: node scripts/v8-v9-diff.mjs [--allow-overwrite] [--v8 PATH] [--v9 PATH]");
      process.exit(0);
    } else {
...
// V9 likely-cause classifier. Inspects reason-token delta + territory_evidence
// to tag the responsible V9-Ax change.
//
// V9-A1 (NA token expansion):
//   - V8 dropped on non_na_territory; V9 keeps. Territory evidence in V8
//     contains a multi-region list with bare US-city abbrevs (SF/NY/LA/DC) OR
//     a coast descriptor (US East Coast / US West Coast / coastal US). The
//     V9 NA-side expansion flipped the body-tie outcome from NON_NA → NA or
//     UNKNOWN.
//
// V9-A2 (location-section anchor):
//   - V8 left as UNKNOWN/keep; V9 drops on non_na_territory because the JD
//     has a markdown `## Location` block (parseJdSections labels the body as
//     `location` type). V9-2 promotes location-typed section bodies (≤120
//     chars) to implicit anchor evidence so anchor-count majority can decide.
export function classifyV9Cause(fromRecord, toRecord, changes) {
  ...
  // Use territory_evidence (Shadow Decisions sheet) which preserves the full
  // detector evidence — NOT hard_drop_evidence which only exists for rows
  // that drop. Both records carry their own territory_evidence column.
  ...
  // V9-A1: was V8 territory-drop with multi-region+abbrev or coast-descriptor
  // evidence, now V9 keeps (or stays drop with different reason).
  // Evidence shape this targets:
  //   - bare US-city abbrev in parens-or-comma list ("(SF, NY, London")
  //   - coast descriptor "US East", "US West", "east coast", "west coast",
  //     "coastal us"
  const v8DroppedTerritory = oldHard && /non_na_territory/.test(oldReason);
  const v9NoLongerTerritory = !/non_na_territory/.test(newReason);
  const oldEvidenceHasV9A1Shape =
    /\((?:sf|ny|la|dc)[,\s]/i.test(oldTerrEvidence) ||
    /[,\s](?:sf|ny|la|dc)\s*[,\)]/i.test(oldTerrEvidence) ||
    /\bus\s+east\b/i.test(oldTerrEvidence) ||
    /\bus\s+west\b/i.test(oldTerrEvidence) ||
    /\b(?:east|west)\s+coast\b/i.test(oldTerrEvidence) ||
    /\bcoastal\s+us\b/i.test(oldTerrEvidence);
  if (v8DroppedTerritory && v9NoLongerTerritory && oldEvidenceHasV9A1Shape) {
    causes.add("V9-A1_na_token_expansion");
  }

  // V9-A2: was V8 keep with UNKNOWN territory, now V9 drops on non_na_territory.
  // Marker: territory_region transitioned UNKNOWN → NON_NA AND new evidence
  // contains a location-section anchor token (`location-section:`).
  const v9NewlyTerritory = !oldHard && newHard && delta.added.includes("non_na_territory");
  const newEvidenceHasLocationAnchor = /\blocation-section:/i.test(newTerrEvidence);
  const territoryUnknownToNonNa = oldTerritory === "UNKNOWN" && newTerritory === "NON_NA";
  if ((v9NewlyTerritory || territoryUnknownToNonNa) && newEvidenceHasLocationAnchor) {
    causes.add("V9-A2_location_section_anchor");
  }
  // V9-A2 broader: any territory UNKNOWN → NON_NA flip is by construction
  // attributable to the V9-2 location-section anchor (no other V9 change
  // moves territory in that direction). Catch even when the evidence string
  // doesn't include the literal "location-section:" marker (e.g. when the
  // anchor capture matched a non-location pattern).
  if (territoryUnknownToNonNa && v9NewlyTerritory) {
    causes.add("V9-A2_location_section_anchor");
  }
...
function classifyChange(fromRecord, toRecord, changes) {
  ...
  const v9Causes = classifyV9Cause(fromRecord, toRecord, changes);
  return {
    change_type: changeTypes.join("; "),
    v9_cause: v9Causes.join("; "),
  };
}
...
function diffVersions(fromVersion, toVersion) {
  ...
  for (const key of [...allKeys].sort()) {
    ...
    const { change_type, v9_cause } = classifyChange(fromRecord, toRecord, changes);
...
function buildCompanyPivot(rows) {
  return [...groupBy(rows, (r) => r.company).entries()]
    .map(([company, group]) => ({
      ...
      v9a1_count: countWhere(group, (r) => /V9-A1/.test(r.v9_cause)),
      v9a2_count: countWhere(group, (r) => /V9-A2/.test(r.v9_cause)),
      other_count: countWhere(group, (r) => /OTHER/.test(r.v9_cause)),
      titles: group.map((r) => r.title).join(" | "),
    }))
    .sort((a, b) => b.changed_rows - a.changed_rows || a.company.localeCompare(b.company));
}

function buildSummaryMetrics(v8, v9, diff, unmatchedRows) {
  const changed = diff.changedRows;
  return [
    { metric: "from_shadow_rows", value: v8.counts.shadow_rows },
    { metric: "to_shadow_rows", value: v9.counts.shadow_rows },
    { metric: "shadow_rows_dedup_delta", value: v9.counts.shadow_rows - v8.counts.shadow_rows },
    ...
    { metric: "v9_a1_attributed_rows", value: countWhere(changed, (r) => /V9-A1/.test(r.v9_cause)) },
    { metric: "v9_a2_attributed_rows", value: countWhere(changed, (r) => /V9-A2/.test(r.v9_cause)) },
    { metric: "v9_other_unattributed_rows", value: countWhere(changed, (r) => /OTHER/.test(r.v9_cause)) },
    ...
    { metric: "from_hard_drops", value: v8.counts.hard_drops },
    { metric: "to_hard_drops", value: v9.counts.hard_drops },
    { metric: "net_hard_drop_delta", value: v9.counts.hard_drops - v8.counts.hard_drops },
    { metric: "from_sales_hard_drops", value: v8.counts.sales_hard_drops },
    { metric: "to_sales_hard_drops", value: v9.counts.sales_hard_drops },
    { metric: "from_territory_hard_drops", value: v8.counts.territory_hard_drops },
    { metric: "to_territory_hard_drops", value: v9.counts.territory_hard_drops },
    { metric: "from_validation_findings", value: v8.counts.validation_findings },
    { metric: "to_validation_findings", value: v9.counts.validation_findings },
...
export async function buildVersionDiff(flags) {
  const versions = {
    v8: await readVersion("v8", flags.versions.v8),
    v9: await readVersion("v9", flags.versions.v9),
  };
  const diff = diffVersions(versions.v8, versions.v9);
  const summaryMetrics = buildSummaryMetrics(versions.v8, versions.v9, diff, diff.unmatchedRows);
  const companyPivot = buildCompanyPivot(diff.changedRows);

  const wb = new ExcelJS.Workbook();
  wb.creator = "career-ops shadow v8-v9 diff";
  wb.created = new Date("2026-05-06T00:00:00-04:00");
  ...
  addSheet(wb, "V8 to V9 Changed Rows", diff.changedRows, changedCols);
  ...
  addSheet(wb, "V9-A1 NA Token Expansion", diff.changedRows.filter((r) => /V9-A1/.test(r.v9_cause)), changedCols);
  addSheet(wb, "V9-A2 Location Anchors", diff.changedRows.filter((r) => /V9-A2/.test(r.v9_cause)), changedCols);
  ...
  addSheet(wb, "Company Role Pivot", companyPivot, ["company", "changed_rows", "hard_drop_added", "hard_drop_removed", "hard_drop_reason_changed", "v9a1_count", "v9a2_count", "other_count", "titles"]);
