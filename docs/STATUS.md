...
## Handoff Note
**Shadow filter calibration arc CLOSED at V10. Will manually reviewed and approved.** Four checkpoint commits landed on `feat/phase-2.8-firecrawl`: framework upgrade (`d73638b`) → test infra + enrichment (`17251c8`) → audit trail (`7dd512e`) → memory + STATUS (this commit). **Next agent action: production wiring** — port V10 rules from `scripts/lib/job-fit-rules.mjs` into `career-ops/export-jobs.mjs`, smoke test against today's pipeline, tag `production-v10`. Estimated 1-2 hours. Reversible via `git revert`. Phase 2.8 baseline preserved at `75ec403` / tag `phase-2.8-complete`. Pre-rescan baseline preserved at tag `scan-v2-prerescan`. After production wiring lands: merge `feat/phase-2.8-firecrawl` → `main`. Phase 3 candidates (LLM eval pipeline, calibration round, delta detection, V11 source-hygiene patch, NO_RELEVANT_JOBS roster cleanup) await Will's selection. Next agent picking up: read `AI_HANDOFF.md` → `.claude/memory/state.md` → this file. Local-only files NOT committed (intentional): `.claude/settings.local.json`, `docs/audits/*test*.json` (test outputs), `career-ops/tmp-extract-territory.mjs` (cleanup-pending temp).
